...
async function installApp() {
    if (deferredPrompt) {
        ...
        if (userData.userId) {
            ...
            // Принудительно сохраняем данные в localStorage
            localStorage.setItem('userData', JSON.stringify(userData));
            ...
            console.log('✅ Данные сохранены для PWA:', userData);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Если это PWA и данные восстановлены, обновляем профиль еще раз
    if ((window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true) && restoredData) {
        console.log('🔄 PWA режим: принудительное обновление профиля');
        setTimeout(() => {
            updateUserProfile();
        }, 100);
        
        // Дополнительное обновление через 500мс для надежности
        setTimeout(() => {
            updateUserProfile();
        }, 500);
    }
...
function restoreUserData() {
    ...
    console.log('  localStorage userData:', savedData ? JSON.parse(savedData) : 'нет');
    console.log('  PWA install данные:', pwaInstallData ? JSON.parse(pwaInstallData) : 'нет');
    console.log('  PWA user данные:', pwaUserData ? JSON.parse(pwaUserData) : 'нет');
    ...
    // Сначала проверяем основные данные
    if (savedData) {
        try {
            userData = JSON.parse(savedData);
            console.log('✅ Данные найдены в userData:', userData);
        } catch (error) {
            console.error('❌ Ошибка парсинга userData:', error);
        }
    }
    
    // Если нет основных данных, проверяем PWA данные
    if (!userData && pwaUserData) {
        try {
            userData = JSON.parse(pwaUserData);
            console.log('✅ Данные найдены в pwaUserData:', userData);
        } catch (error) {
            console.error('❌ Ошибка парсинга pwaUserData:', error);
        }
    }
    
    // Если нет PWA данных, проверяем install данные
    if (!userData && pwaInstallData) {
        try {
            const installData = JSON.parse(pwaInstallData);
            userData = {
                userId: installData.userId,
                name: installData.name,
                phone: installData.phone
            };
            console.log('✅ Данные найдены в pwaInstallData:', userData);
        } catch (error) {
            console.error('❌ Ошибка парсинга pwaInstallData:', error);
        }
    }
    ...
    if (userData && userData.userId) {
        ...
        // Сохраняем данные в основном хранилище
        localStorage.setItem('userData', JSON.stringify(userData));
        console.log('✅ Данные сохранены в userData');
