...
async function checkActiveOrder() {
    ...
    try {
        const response = await fetch(`http://localhost:5000/api/user_orders/${userData.userId}`);
...
function showDriverAssigned(order) {
    ...
    successContent.innerHTML = `
        ...
        <div class="driver-info">
            ...
            <p><strong>Подача машины:</strong> 5-7 минут</p>
        </div>
        <button class="cancel-order-btn" onclick="cancelOrder()">Отменить заказ</button>
    `;
...
async function cancelOrder() {
    ...
    try {
        const response = await fetch(`http://localhost:5000/api/cancel_order/${currentOrder}`, {
            method: 'POST'
        });
...
function startOrderTracking(orderId) {
    ...
    orderCheckInterval = setInterval(async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/order_status/${orderId}`);
            const result = await response.json();
